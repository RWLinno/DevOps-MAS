...
"""
Search Agent
Real-time web search and content extraction for DevOps-MAS scenarios
...
class SearchAgent(BaseAgent):
    ...
    async def _extract_webpage_content(self, url: str) -> Optional[str]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                headers = {
                    'User-Agent': 'Mozilla/5.0 (compatible; DevOps-MAS/1.0)'
                }
    ...
    def _prioritize_results(self, results: List[Dict[str, Any]], query: str) -> List[Dict[str, Any]]:
        """Prioritize search results based on DevOps-MAS relevance"""
    ...
    async def _summarize_search_results(self, results: List[Dict[str, Any]], query: str) -> str:
        ...
        try:
            ...
            prompt = f"""Based on the following search results, provide a concise and actionable answer for the DevOps-MAS engineer's question.
...
4. Any urgent considerations for DevOps-MAS scenarios
